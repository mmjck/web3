...
contract ChatApp {
    ...
    function checkUserExists(address _addr) public view returns (bool) {
        return bytes(userList[_addr].name).length > 0;
    }

    function createAccount(string calldata _name) external {
        require(checkUserExists(msg.sender) == false, "User already existx");
    ...
    function getUsername(address _addr) external view returns (string memory) {
        require(checkUserExists(_addr), "USer is not registered");
